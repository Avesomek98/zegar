...
- **`app_settings`** — one row per user. Notable columns:
  ...
  - `schedule` (jsonb) — holds **both** `customPresets` (the user's saved shift templates) and `extraPolandRanges` (ad-hoc Poland stays outside the normal cycle) in one column, added later without a schema migration. Any write to this column must include the current value of both sub-keys, or it silently clobbers the other one — this actually happened in production (a stale/never-loaded local cache for one sub-key got written back over real data for the other). `saveCustomPresets`/`saveExtraPolandRanges` in script.js now both call `fetchCurrentSchedule()` to re-read the *current* DB value of the sub-key they're not touching immediately before every write, rather than trusting the in-memory cache — don't reintroduce a version that merges from the local cache instead.
